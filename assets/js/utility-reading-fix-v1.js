(() => {
  ...
  style.textContent = `
    ...
    .ur-prev{background:#eef8f3;border-radius:10px;padding:12px;color:#33584c;font-size:13px}.ur-prev b{display:block;color:#003c41;font-size:16px;margin-top:3px}
  ...
  document.addEventListener('click',e=>{
    ...
    if(text.includes('registrar leitura')){
      ...
      const card=button.closest('.card,.panel,.dashboard-card,.utility-card,article,section,div');
      const title=card?.querySelector('h1,h2,h3,h4,h5,strong,b')?.textContent?.trim()||'';
      openReadingModal().catch(err=>toast(err.message||'Erro ao abrir registro.',true));
    }
  },true);
})();
