...
(function () {
  ...
  function toast(message, kind = 'info') {
    const old = document.querySelector('.bt-app-toast');
    if (old) old.remove();
  ...
  function showLoginError(root, message) {
    ...
    if (!el) {
      ...
      const wrap = root.querySelector('.right-inner') || root;
      wrap.appendChild(el);
    }
  ...
  async function hydrateCurrentUser(session) {
    ...
    let profile = null;
    try {
      const result = await sb.from('profiles').select('id,name,full_name,email,phone,sector,department,role,avatar_url').eq('id', userId).maybeSingle();
    ...
    state.currentUser = {
      ...
      usuario: session.user.email || profile?.email || '',
      ...
      role_code: profile?.role || 'tecnico',
      perfil: profile?.role || 'tecnico',
      avatar_url: profile?.avatar_url || null,
      phone: profile?.phone || ''
    };

    try {
      if (state.screen === 'login' || !state.screen) state.screen = 'app';
      if (!state.activeArea) state.activeArea = 'home';
  ...
  async function doLogin(ui) {
    const sb = getSB();
    if (!sb) return showLoginError(ui.root, 'Backend de autenticação indisponível.');
    ...
    showLoginError(ui.root, '');

    try {
      const { data, error } = await sb.auth.signInWithPassword({ email, password });
      if (error) throw error;
      if (!data?.session) throw new Error('O Supabase não retornou uma sessão válida.');
      const ok = await hydrateCurrentUser(data.session);
      if (!ok) throw new Error('Sessão criada, mas o perfil não pôde ser carregado.');
      ...
      const friendly = /invalid login credentials/i.test(raw)
        ...
        : raw;
      showLoginError(ui.root, friendly);
  ...
  function bindLogin() {
    const ui = findLoginInputs();
    if (!ui || ui.submit.dataset.btAuthBound === '1') return !!ui;
    ui.submit.dataset.btAuthBound = '1';

    ui.submit.addEventListener('click', (event) => {
  ...
  async function bootstrapAuth() {
    const sb = getSB();
    if (!sb || sessionBootstrapDone) return;
    sessionBootstrapDone = true;
    try {
      const { data } = await sb.auth.getSession();
      if (data?.session) await hydrateCurrentUser(data.session);
      sb.auth.onAuthStateChange((_event, session) => {
        if (session) hydrateCurrentUser(session);
      });
    } catch (error) {
      console.warn('BIOTROP auth bootstrap:', error);
    }
  }
  ...
  function startLoginWatcher() {
    ...
    loginWatcher = window.setInterval(() => {
      ...
      if (ticks > 150 || document.querySelector('.shell')) {
        window.clearInterval(loginWatcher);
        loginWatcher = null;
      }
    }, 400);
  }
